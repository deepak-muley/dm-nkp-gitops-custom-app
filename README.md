...
- **Metrics** → Prometheus
- **Logs** → Loki (via OTLP - requires Loki 3.0+)
- **Traces** → Tempo
...
│  │  - GET /        → JSON response + metrics + trace                   │    │
│  │  - GET /health  → Liveness probe + trace                            │    │
│  │  - GET /ready   → Readiness probe + trace                           │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                             │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐             │
...
│  ┌───────────────┐  │  │  ┌───────────────┐  │  │  ┌───────────────┐  │
│  │ Metrics       │  │  │  │ Logs          │  │  │  │ Traces        │  │
│  │ - http_*      │  │  │  │ - OTLP logs   │  │  │  │ - Spans       │  │
│  │ - business_*  │  │  │  │ - FluentBit   │  │  │  │ - service.name│  │
│  └───────────────┘  │  │  │   logs        │  │  │  │ - http.target │  │
│  Port: 9090         │  │  └───────────────┘  │  │  └───────────────┘  │
└──────────┬──────────┘  │  Port: 80 (gateway) │  │  Port: 3200/4317   │
           │             └──────────┬──────────┘  └──────────┬──────────┘
           │                        │                        │
           └────────────────────────┼────────────────────────┘
                                    │
...
Logs are collected via **two methods**:
...
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │  Application Code                                                   │   │
│   │  - telemetry.LogInfo(ctx, "message")  → OTel SDK + stdout           │   │
│   │  - telemetry.LogError(ctx, "msg", err) → OTel SDK + stderr          │   │
│   │  - telemetry.LogWarn(ctx, "message")  → OTel SDK + stderr           │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                    │                                │                       │
│                    │ OTLP (gRPC)                    │ stdout/stderr         │
│                    ▼                                ▼                       │
└────────────────────┼────────────────────────────────┼───────────────────────┘
                     │                                │
     ┌───────────────┘                                └───────────────┐
...
│   (OTLP Receiver)           │                    │   (FluentBit DaemonSet)     │
│                             │                    │                             │
│   Labels:                   │                    │   Labels:                   │
│   - service_name            │                    │   - namespace               │
│   - severity_text (INFO,    │                    │   - pod                     │
│     ERROR, WARN)            │                    │   - container               │
│   - trace_id (correlation)  │                    │   - app_kubernetes_io_name  │
└──────────────┬──────────────┘                    └──────────────┬──────────────┘
               │ otlphttp                                         │ HTTP Push
...
│                                                                             │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │  HTTP Handler with otelhttp Middleware                              │   │
│   │                                                                     │   │
│   │  Auto-instrumented spans:                                           │   │
│   │  - HTTP request: method, url, status_code, client_ip                │   │
│   │                                                                     │   │
│   │  Manual spans:                                                      │   │
│   │  - process.request                                                  │   │
│   │  - business.logic                                                   │   │
...
│                      OpenTelemetry Collector                               │
│                                                                            │
│   Processors add resource attributes:                                      │
│   - service.name = "dm-nkp-gitops-custom-app"                              │
│   - job = "otel-collector"                                                 │
│                                                                            │
│   Exporter: otlp/tempo                                                     │
└────────────────────────────────┬───────────────────────────────────────────┘
...
### End-to-End Demo with OpenTelemetry

To see the complete setup with metrics, logs, and traces in Grafana:

```bash
# Run the full E2E demo with OpenTelemetry stack
...
- OpenTelemetry Collector
- Prometheus (metrics)
- Loki 3.0+ (logs with OTLP support)
- Tempo (traces)
